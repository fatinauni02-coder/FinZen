...
document.addEventListener('DOMContentLoaded', function () {
    ...
    function createGoalCard(goal) {
        ...
        if(isCompleted) statusC = "style = 'background-color: #E0E0E0'";
        const cardHTML = `
            <div class="col col-12 col-lg-6 p-1 goal-card">
                <div class="card" 
                    data-id="${dataHandle(goal.id)}"
    ...
    function renderData(goal){
        ...
        goalRemaining.value = calRemaining(goal);
    ...
    positiveBtn.addEventListener('click', async function(){
        if (currentMode === 'view') {
            ...
            try {
                ...
                if(currentMode === 'edit')
                    await GoalAPI.update(currentEditingId, newData);
                if(currentMode === 'add')
                    await GoalAPI.create({ ...newData, saved: 0 });
                goalsList = await GoalAPI.fetchAll();
                renderAll(goalsList);
                modalInstance.hide();
            } catch (error) {
    ...
    function modeController(mode, goalId=null){
        ...
        let goal = goalsList.find(g => String(g.id) === String(currentEditingId));
    ...
    function updateGoalSelection(goalsList) {
        ...
        goalsList.forEach(goal => {
            const optionHTML = `<option value="${dataHandle(goal.id)}">${dataHandle(goal.name)} need ${calRequired(goal)}</option>`;
    ...
    manualSavedBtn.addEventListener('click',async function(event){
        ...
        if(isNaN(budget) || !(selectedId) || budget<0){
            ...
            try{
                ...
                let goal = goalsList.find(g => String(g.id) === String(selectedId));
                if(goal != null){
                    const final = Math.min(goal.target,goal.saved+budget);
                    if(final >= goal.target){
                        alert('Goal completed!');
                    }
                    await GoalAPI.update(selectedId,{saved: final});
    ...
    async function initialize() {
        try {
            const [goalData,userData] = await Promise.all([GoalAPI.fetchAll(),GoalAPI.fetchProfile()]);
            goalsList = goalData;
